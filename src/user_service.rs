...
pub fn retrieve_user(context: impl LMContext,
                     source: &str,
                     source_id: &str) -> Option<User> {
...
mod tests {
    ...
    fn test_retrieve_user() {
        ...
        assert_eq!("AT1-FIVE-SIX".to_string(), u5.user_accounts[0].account_source_id);
        assert_eq!("AT1A".to_string(), u5.user_accounts[1].account_type.name);
        assert_eq!("AT2 SOURCE".to_string(), u5.user_accounts[2].account_type.source);

        let u6 = retrieve_user(us_context(), "source", "source-id-6").unwrap();
        assert_eq!(6, u6.id);
        assert_eq!("User Six", u6.name);
        assert_eq!(3, u6.user_accounts.len());
        assert_eq!("AT1-FIVE-SIX".to_string(), u6.user_accounts[0].account_source_id);
        assert_eq!("AT1A-FIVE-SIX".to_string(), u6.user_accounts[1].account_source_id);
    ...
    fn user(id: u64, name: String, source: String, source_id: String, user_accounts: Vec<Account>) -> User {
        User {
            id,
            name,
            source,
    ...
    fn users() -> Vec<User> {
        let u5_accounts = vec![user_account(55, at1(), "AT1-FIVE-SIX".to_string()),
                               user_account(56, at1a(), "AT1A-FIVE-SIX".to_string()),
                               user_account(57, at2(), "AT2-FIVE".to_string())];
        let u6_accounts = vec![user_account(65, at1(), "AT1-FIVE-SIX".to_string()),
                               user_account(66, at1a(), "AT1A-FIVE-SIX".to_string()),
                               user_account(67, at2(), "AT2-SIX".to_string())];
        vec![user(5, "User Five".to_string(), "source".to_string(), "source-id-5".to_string(), u5_accounts),
             user(6, "User Six".to_string(), "source".to_string(), "source-id-6".to_string(), u6_accounts),
        ]
    }
