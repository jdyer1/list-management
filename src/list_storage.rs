...
use crate::common::{Account, AccountType, ItemList, ListAccess, ListAttribute, ListItem, ListStorage, ListType, Price, UserState};
use crate::db;
use crate::models::{AccountDb, AccountTypeDb, ItemListAccountDb, ItemListAttributeDb, ItemListDb, ListItemAttributeDb, ListItemDb};
use crate::schema::{account, account_type, item_list, item_list_attribute, list_item, list_item_attribute};
...
fn get_lists(lists: Vec<ItemListDb>) -> Vec<ItemList> {
    ...
    let accounts_per_lists: HashMap<i32, Vec<Account>> =
        accounts.grouped_by(&lists)
            .into_iter().zip(&lists).map(|(vec_of_iladb_adb, ildb)|
            (ildb.id, vec_of_iladb_adb.into_iter().map(|(_, adb)|
                Account {
                    id: adb.id as u64,
                    account_type: account_types_by_id.get(&adb.account_type_id).unwrap_or(&AccountType {
...
fn all_account_types() -> HashMap<i32, AccountType> {
    ...
    let v = account_type::table.select(AccountTypeDb::as_select()).load(&mut c).unwrap();
    for atdb in v {
        let id = atdb.id;
        let at = AccountType {
            id: id as u64,
...
mod tests {
    ...
    fn test_all_lists() {
        ...
        assert_eq!(2, a[0].list_accounts.len());
        assert_eq!("at source 1".to_string(), a[0].list_accounts[0].account_type.source);
        assert_eq!("account type 1".to_string(), a[0].list_accounts[0].account_type.name);
        assert_eq!("at source 1 one".to_string(), a[0].list_accounts[0].account_source_id);
    ...
    fn test_user_lists() {
        let mut user_ids = (0,0);
        {
            ...
            let users_vec = user::table.select(UserDb::as_select()).order(user::id.asc()).load(c).unwrap();
    ...
    fn insert_item_list_account(c: &mut PooledConnection<ConnectionManager<SqliteConnection>>, item_list_id: i32, account_id: i32) {
        diesel::insert_into(item_list_account::table)
            .values((item_list_account::item_list_id.eq(&item_list_id), item_list_account::account_id.eq(&account_id)))
            .execute(c).expect("Could not insert item_list_account");
    }
}